...
| **2. Choose brush size** | Use **1 (8 px)**, **2 (16 px)**, **4 (32 px)**, **8 (64 px)**, or **16 (128 px)**. Select the smallest size that lets you mark the decision region clearly. |
...
| **5. Classify and save** | Select **Ischemic** or **Non-ischemic**. Use **Preview final mask** for a clean white-overlay check, then select **Save & next image**. After the final image, the app confirms that the session is complete. |
...
Images are presented in one saved random order for this session. If the application is reopened during the same session, it restores that same order. A new Study Kit/session uses a new random order.
